...
# 6) FSII approx
    ...
    # === UpSet Plot (2nd-Order Interactions, echte Feature-IDs) ===
    ...
    for (i, j), val in zip(top10_pairs, top10_values):
        print(f"non shapiq: Interaction ({i}, {j}): {val:.4f}")

    # memberships = echte Feature-Paare als Strings
    memberships = [(f"Feature {i}", f"Feature {j}") for (i, j) in top10_pairs]


    df = from_memberships(memberships, data=top10_values)

    # Plot erstellen
    fig = plt.figure(figsize=(10, 5))
    upset = UpSet(df, show_counts=True, show_percentages=False)
    upset.plot(fig)
    plt.title(f"UpSet Plot – Top 10 echte Features ({model_name})", fontsize=8, fontweight='bold', color='navy')
    plt.tight_layout()
    ...
    # 1. Bild lokal speichern für Kontrolle
    safe_model_name = model_name.replace("/", "_")
    upset_img.save(f"upset_{safe_model_name}.png")

    # 2. Croppen (z. B. 150px links weg)
    cropped = upset_img.crop((173, 0, upset_img.width, upset_img.height))

    # 3. Optional: Gecropptes Debugbild speichern
    cropped.save(f"upset_cropped_{safe_model_name}.png")

    # 4. In finale Liste einfügen
    upset_plots_per_model.append(cropped)
    plt.close(fig)
