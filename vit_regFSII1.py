...
# 6. Show the first 6 actually sampled coalitions for sanity checking.
...
def _no_border(self, sampling_budget: int) -> int:
    # never apply SHAPIQ's internal "border-trick", keep full budget
...
# 5.1 define dummy weights & value_fn for sampler
w = np.zeros(n_patches+1)
w[1], w[2] = 0.2, 0.8   # 20% singles, 80% pairs
w /= w.sum()

# value function placeholder until ViT loads
def value_fn(coalitions: list[np.ndarray]) -> np.ndarray:
    # we won't actually call it here, sampler only needs shape
...
_ = small_approx.approximate(
    budget=50,            # tiny demo budget
    game=value_fn,
    return_interactions=True
)

coalitions = small_approx._sampler.coalitions_matrix  # shape (50, 49)
...
# 7. Redefine value function properly
def value_fn(coalitions: list[np.ndarray]) -> np.ndarray:
    out = []
...
# 8. Quick full vs empty check
print("Logit (full): ",  value_fn([np.ones(n_patches, bool)])[0])
print("Logit (empty):",  value_fn([np.zeros(n_patches, bool)])[0])
...
# **Notes**
# - **Monkey-Patch** disables SHAPIQ’s “border-trick” to avoid dimension errors.  
# - **sampling_weights** controls the fraction of budget drawn for single vs. pair masks.  
# - **return_interactions=False** returns only the final `dict_values` of FSII scores.
