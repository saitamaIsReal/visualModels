...
def main():
    ...
    # ein paar Test-Koalitionen anlegen
    test_coalitions = []
    # 1) Vollbild
    test_coalitions.append(np.ones(n_patches, dtype=bool))
    # 2) Komplett maskiert
    test_coalitions.append(np.zeros(n_patches, dtype=bool))
    # 3) Drei Single-Patch Masken
    for i in [0, 10, 20]:
        m = np.ones(n_patches, dtype=bool)
        m[i] = False
        test_coalitions.append(m)
    # 4) Zwei Paar-Masken
    for (i, j) in [(0, 1), (2, 3)]:
        m = np.ones(n_patches, dtype=bool)
        m[i] = m[j] = False
    ...
    # die maskierten Bilder speichern
    for idx, coalition in enumerate(test_coalitions):
        masked_img = mask_image_grid(image, coalition)
        masked_img.save(f"coalitions/coalition_{idx}.png")
        print(f"Saved coalitions/coalition_{idx}.png")
    ...
    # greif auf die zuletzt gesampelten Koalitionen zu
    coalitions = approximator._sampler.coalitions_matrix  # Shape: (budget, n_patches)

    for idx, coalition in enumerate(coalitions[:10]):
        masked_img = mask_image_grid(image, coalition)
        path = os.path.join("coalitions", f"coalition_{idx}.png")
